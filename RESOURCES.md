...
| 🔒 ROS 2 CLI cheat sheet PDF (KD → Learnings → ROS 2). **2019 edition:** in Humble `ros2 msg`/`ros2 srv` are gone (use `ros2 interface show`), and its bag examples are missing the word `bag` | M4 |
| 🔒 *Fundamentals of Control Theory* book (`fundamentals_of_control_r1_6.pdf`, KD → Learnings → Control Systems). Focus: PID and LQR chapters | M3, L1 |
| 🔒 `StateSpace.pdf` (State-space representation of LTI systems) | L1 |
| 🔒 `lqr.pdf` (R. M. Murray, Lecture 2 – LQR Control) | L1 |
