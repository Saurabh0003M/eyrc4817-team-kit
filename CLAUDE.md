...
basic Python). His goal is **learning**. **The PDF roadmap was dropped (2026-09-17: he didn't like it); the team now learns through a shared ChatGPT Project** built from `chatgpt-project/`. **Task 1 (23 Sep): KD 1A + PB 1A done and packed (Claude wrote both, at Saurabh's request, time/tokens short); KD 1B/1C and PB 1B not done** — details in `context/04-task-log.md`. Claude does setup and
